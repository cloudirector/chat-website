...
def sha256(inputdata):
    ...
    for i in range(0, len(inputdata), 64):
        ...
        for j in range(16, 64):
            s0 = (words[j-15] >> 7 ^ words[j-15] >> 18 ^ words[j-15] >> 3)
            s1 = (words[j-2] >> 17 ^ words[j-2] >> 19 ^ words[j-2] >> 10)
...
for i in range(1, 1001):
    i_str = str(i)
    print(f"{i}:{sha256(i_str)}")
end_time = time.time()
elapsed_time = end_time - start_time
hashes_per_second = 1000 / elapsed_time
print(f"took {elapsed_time:.4f} seconds at {hashes_per_second:.2f} h/s")